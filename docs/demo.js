...
function renderModels(){const provider=$('#provider-select').value;$('#model-select').innerHTML=models[provider].map(model=>`<option>${model}</option>`).join('');$('#availability-note').textContent=`${labels[provider]} · ${models[provider][0]}`}
...
document.addEventListener('DOMContentLoaded',()=>{
  ...
  $('#model-select').addEventListener('change',()=>$('#availability-note').textContent=`${labels[$('#provider-select').value]} · ${$('#model-select').value}`);
  $('#references').addEventListener('change',event=>{$('#reference-preview').innerHTML=[...event.target.files].slice(0,14).map((file,index)=>`<figure><img src="${URL.createObjectURL(file)}"><figcaption>${index+1}</figcaption></figure>`).join('')});
