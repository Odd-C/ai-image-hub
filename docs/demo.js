...
function addGeneration(x,y,inherit={}){const created=defaultGeneration(x,y,inherit);state.nodes.push(created);select(created.id,false);save();requestAnimationFrame(()=>$(`[data-id="${created.id}"] textarea`)?.focus());return created}
...
function inputRows(n){return n.inputs.map((ref,index)=>{const source=resolveRef(ref);if(!source)return `<li class="missing" data-input="${esc(ref)}" data-owner="${n.id}"><span>图${index+1} 已失效</span><button data-remove="${esc(ref)}" aria-label="移除图${index+1}">×</button></li>`;return `<li draggable="true" data-input="${esc(ref)}" data-owner="${n.id}"><span>图${index+1}</span><img src="${esc(source.src)}" alt=""><button data-remove="${esc(ref)}" aria-label="移除图${index+1}">×</button></li>`}).join('')}
...
function rail(n){
  ...
  return `<footer class="generation-rail"><span title="${esc(`${ok}/${results.length} 张结果`)}">${ok}/${results.length} 张结果${n.error?' · 有失败':''}</span><div class="image-actions"><div class="file-actions"><button data-open-result ${ready?'':'disabled'} aria-label="打开主图">${icons.open}</button><button data-download-result ${ready?'':'disabled'} aria-label="下载主图">${icons.download}</button></div><button class="edit-action" data-edit aria-expanded="${n.expanded}">编辑并重新生成</button><div class="remove-actions"><button data-delete aria-label="从画布移除">${icons.remove}</button></div></div></footer>`;
}
function generationMarkup(n){
  const active=batchResults(n.activeBatch).length?n.activeBatch:null,batch=active||(batchResults(n.attempt).length?n.attempt:null);
  return `<article class="node generation${n.expanded?' expanded':''} ${state.selected.includes(n.id)?'selected':''}" data-id="${n.id}" data-node-type="generation_node" aria-selected="${state.selected.includes(n.id)}" tabindex="-1" style="left:${n.x}px;top:${n.y}px;width:${n.w}px"><header><div class="generation-heading"><b>生成</b><small>${n.inputs.length} 张输入 · ${esc(statusText(n))}</small><em class="dirty-flag" ${n.dirty?'':'hidden'}>待重新生成</em></div><div class="node-header-actions"><button data-toggle aria-label="${n.expanded?'收起为结果简洁态':'展开编辑参数'}" aria-expanded="${n.expanded}">${icons.chevron}</button><i></i><button class="request-remove" data-delete aria-label="从画布移除">${icons.remove}</button></div></header><div class="generation-body"><button class="port in" aria-label="参考图输入端口"></button><div class="generation-media">${batch?grid(n,batch,Boolean(active)):''}${progress(n)}${errorRow(n,batch)}</div>${n.expanded?editor(n):''}</div>${rail(n)}</article>`;
}
...
function toggleGeneration(n){if(!isGeneration(n))return;n.expanded=!n.expanded;render();requestAnimationFrame(()=>requestAnimationFrame(()=>{links();minimap()}));save()}
function activateResult(n,batch,result){let changed=false;const results=batchResults(batch);if(results.length>1&&n.primaryResultId!==result.id){n.primaryResultId=result.id;changed=true}if(!n.expanded){n.expanded=true;changed=true}if(changed)render();save()}
...
function settleAttempt(n){
  ...
  if(ok||!previous.length){
    ...
    n.dirty=isDirty(n);
    if(!ok)n.expanded=true;else n.expanded=false;
    return true;
  }
...
function generate(n){
  ...
  n.attempt={id:uid('batch'),request,results,settled:false};n.submitting=true;n.error='';render();save();
...
document.addEventListener('DOMContentLoaded',()=>{
  ...
  vp.onpointerdown=e=>{
    ...
    drag={pointer:e.pointerId,sx:e.clientX,sy:e.clientY,origins:state.selected.map(id=>({id,x:node(id).x,y:node(id).y})),moved:false};
    state.selected.forEach(id=>$(`[data-id="${id}"]`)?.classList.add('dragging'));
    vp.setPointerCapture?.(e.pointerId);syncDragging();
  };
  vp.onpointermove=e=>{
    ...
    else if(drag&&e.pointerId===drag.pointer){const dx=(e.clientX-drag.sx)/state.view.z,dy=(e.clientY-drag.sy)/state.view.z;if(Math.hypot(dx,dy)>2)drag.moved=true;drag.origins.forEach(o=>{const n=node(o.id);if(!n)return;n.x=o.x+dx;n.y=o.y+dy;const el=$(`[data-id="${n.id}"]`);if(el){el.style.left=`${n.x}px`;el.style.top=`${n.y}px`}});links()}
  ...
  $('#nodes').oninput=e=>{
    ...
    if(root){const flag=$('.dirty-flag',root),note=$('.dirty-note',root);if(flag)flag.hidden=!n.dirty;if(note)note.hidden=!n.dirty}
    save();
  };
  ...
  $('#nodes').onclick=e=>{
    ...
    else if(e.target.closest('[data-edit]')){if(!n.expanded){n.expanded=true;render();save()}}
